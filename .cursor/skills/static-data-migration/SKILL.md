...
|---|---|---|
| Postgres (authoritative) | `server/prisma/schema.prisma` + `server/src/mesaops/*` + `src/mesaops/lib/queries/*` | Target for all operational data |
| Legacy blob | `data.json` + `server/src/legacy/dataJson.ts` (`/api/data`) + legacy App lifted state (retired) | Being retired — write-noise for migrated domains |
| Client static / localStorage | `src/mesaops/mockData.ts`, `src/mesaops/lib/userStore.ts`, `src/mesaops/lib/accessStore.ts` | Demo seed + browser-local state |

`docs/specs/static-and-unlinked-data.md` is the standing inventory report. Read it before starting and update it when you change any plane.
...
Prints `data.json` key row counts, every `src/mesaops/mockData.ts` export with its importers, Prisma models, and flags exports with no model or no importer. Never assume a dataset's status from the report doc alone — the doc can lag the code.
...
**1. Audit.** Run the script. Identify every importer of the `initial*` export, whether the data also flows through `data.json`, and whether `src/mesaops/App.tsx` holds lifted state for it.
...
**4. Server module.** Create `server/src/mesaops/<domain>/` with `schemas.ts` (Zod), `service.ts` (Prisma + `audit`), `router.ts` (`requirePermission` + `validateBody`), and register the router on the authenticated `api` router in `server/src/app.ts`. Services read the tenant from `tenantContext`, never from the request body.

**5. Query hook.** Add `src/mesaops/lib/queries/<domain>.ts` with a local `keys` object, `use{Entity}()` queries and `use{Action}{Entity}()` mutations that invalidate on success.
...
**7. Remove the static source.** Delete the export from `src/mesaops/mockData.ts` once nothing imports it, drop the key from `server/src/legacy/dataJson.ts` `getInitialData()`, remove the lifted state and its sync entry in `src/mesaops/App.tsx` (both the `POST /api/data` path and the `firebaseSync` path), and delete the key from `data.json`.

**8. Verify.** `npm run lint && npm run lint:server`, then `npm run test:unit` and `npm run test:server`. Add a module test at `server/src/mesaops/<domain>/<domain>.test.ts`.
...
2. Remove the `useState` + its entry from the `GET /api/data` hydration and the debounced save in `src/mesaops/App.tsx`.
...
- **Update the report.** Any change to a data plane means updating the affected table in `docs/specs/static-and-unlinked-data.md`.
