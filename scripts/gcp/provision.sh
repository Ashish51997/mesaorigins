...
ensure_database_secret mesadesk-database-url MESADESK_DATABASE_URL_VALUE
ensure_database_secret mesadesk-direct-database-url MESADESK_DIRECT_DATABASE_URL_VALUE
...
if ! gcloud iam service-accounts describe "${MIGRATE_SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com" &>/dev/null; then
  gcloud iam service-accounts create "$MIGRATE_SA_NAME" --display-name="MesaDesk database migrations"
fi
...
if ! gcloud iam service-accounts describe "${BUILD_SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com" &>/dev/null; then
  gcloud iam service-accounts create "$BUILD_SA_NAME" --display-name="MesaDesk release pipeline"
fi
