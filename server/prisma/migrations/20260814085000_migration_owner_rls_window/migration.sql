-- Cloud SQL does not expose PostgreSQL BYPASSRLS to customer-managed users.
-- Keep FORCE RLS enabled and grant cross-tenant access only to the separate
-- table-owning migration identity before the MesaERP foundation backfill.
DO $$
DECLARE
