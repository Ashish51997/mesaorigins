-- Final refresh after statutory profile tables are installed. Every FORCE-RLS
-- tenant table ends the release with the same exact-owner maintenance policy;
-- the runtime app_user remains governed only by tenant_isolation.
DO $$
DECLARE
