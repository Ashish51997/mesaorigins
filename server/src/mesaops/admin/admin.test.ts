...
describe('admin slice', () => {
  it('lists employees + roles and denies a non-admin', async () => {
    const emp = await request(app).get('/api/mesaops/v1/employees');
    expect(emp.status).toBe(200);
    expect(emp.body.length).toBeGreaterThan(10);
    const roles = await request(app).get('/api/mesaops/v1/roles');
    expect(roles.status).toBe(200);
    expect((roles.body as Array<{ name: string }>).some((r) => r.name === 'Operator')).toBe(true);
    // An Operator cannot open the People screen.
    expect((await request(app).get('/api/mesaops/v1/employees').set('x-dev-user', 'EMP-007')).status).toBe(403);
  });
  ...
  it('creates a custom role + employee and enforces access end-to-end', async () => {
    ...
    const role = await request(app).post('/api/mesaops/v1/roles').send({ name: `Floor Lead ${uniq()}`, screens: ['roll_queue', 'holds'] });
    expect(role.status).toBe(201);

    const emp = await request(app).post('/api/mesaops/v1/employees').send({ name: 'Test Lead', email: `lead.${uniq()}@masspolymer.in`, roleId: role.body.id });
    ...
    expect((await request(app).get('/api/mesaops/v1/quality/queue').set('x-dev-user', code)).status).toBe(200);
    expect((await request(app).get('/api/mesaops/v1/formulations').set('x-dev-user', code)).status).toBe(403);

    // Per-employee override: grant formulations → now allowed for this one person.
    const g = await request(app).put(`/api/mesaops/v1/employees/${emp.body.id}/grants`).send({ grants: [{ screen: 'formulations', state: 'on' }] });
    expect(g.status).toBe(200);
    expect((await request(app).get('/api/mesaops/v1/formulations').set('x-dev-user', code)).status).toBe(200);

    // /me/permissions reflects the effective set.
    const me = await request(app).get('/api/mesaops/v1/me/permissions').set('x-dev-user', code);
  ...
  it('reassigning an employee to another role changes their access', async () => {
    const emp = await request(app).post('/api/mesaops/v1/employees').send({ name: 'Mover', email: `mover.${uniq()}@masspolymer.in`, roleId: (await roleId('Operator')) });
    const code = emp.body.employeeCode;
    expect((await request(app).get('/api/mesaops/v1/formulations').set('x-dev-user', code)).status).toBe(403); // Operator can't
    await request(app).patch(`/api/mesaops/v1/employees/${emp.body.id}`).send({ roleId: await roleId('Production Planner') });
    expect((await request(app).get('/api/mesaops/v1/formulations').set('x-dev-user', code)).status).toBe(200); // Planner can
  });

  it('blocks deleting a built-in role; allows an empty custom one', async () => {
    const roles = (await request(app).get('/api/mesaops/v1/roles')).body as Array<{ id: string; name: string; isSystem: boolean }>;
    const builtin = roles.find((r) => r.isSystem)!;
    expect((await request(app).delete(`/api/mesaops/v1/roles/${builtin.id}`)).status).toBe(409);
    const plantScope = roles.find((r) => r.name === 'MesaOps Plant Access')!;
    expect(plantScope.isSystem).toBe(true);
    expect((await request(app).patch(`/api/mesaops/v1/roles/${plantScope.id}`).send({ screens: ['machines'] })).status).toBe(409);
    expect((await request(app).post('/api/mesaops/v1/employees').send({
      name: 'Invalid scope-role employee', email: `scope-role.${uniq()}@masspolymer.in`, roleId: plantScope.id,
    })).status).toBe(409);
    expect((await request(app).post('/api/mesaops/v1/roles').send({ name: 'MesaOps Plant Access', screens: [] })).status).toBe(409);
    const tmp = await request(app).post('/api/mesaops/v1/roles').send({ name: `Temp ${uniq()}`, screens: [] });
    expect((await request(app).delete(`/api/mesaops/v1/roles/${tmp.body.id}`)).status).toBe(200);
  });
  ...
  it('administers only idempotent, versioned MesaOps plant assignments', async () => {
    ...
    const suffix = uniq();
    const role = await request(app).post('/api/mesaops/v1/roles').send({
      name: `Scoped Operator ${suffix}`,
      screens: ['tasks', 'logbook', 'roll_queue', 'inventory'],
    });
    expect(role.status).toBe(201);
    const employee = await request(app).post('/api/mesaops/v1/employees').send({
      name: `Scoped Operator ${suffix}`,
    ...
    expect((await request(app).post('/api/mesaops/v1/role-assignments').send(body)).status).toBe(400);
    expect((await request(app).post('/api/mesaops/v1/role-assignments')
      .set('Idempotency-Key', `scope-service-${suffix}`)
      .send({ ...body, serviceId: 'mesaerp' })).status).toBe(422);

    const createKey = `scope-create-${suffix}`;
    const created = await request(app).post('/api/mesaops/v1/role-assignments')
    ...
    const replay = await request(app).post('/api/mesaops/v1/role-assignments')
      .set('Idempotency-Key', createKey)
      .send(body);
    expect(replay.status).toBe(201);
    expect(replay.body.id).toBe(created.body.id);
    const conflict = await request(app).post('/api/mesaops/v1/role-assignments')
      .set('Idempotency-Key', createKey)
    ...
    expect((await request(app).patch(`/api/mesaops/v1/roles/${erpAssignment!.roleId}`).send({ name: `Forged ERP role ${suffix}` })).body.error.code)
      .toBe('mesaerp_role_forbidden');
    expect((await request(app).delete(`/api/mesaops/v1/roles/${erpAssignment!.roleId}`)).body.error.code)
      .toBe('mesaerp_role_forbidden');
    const erpRoleAttempt = await request(app).post('/api/mesaops/v1/role-assignments')
    ...
    const hiddenErpRevoke = await request(app).post(`/api/mesaops/v1/role-assignments/${erpAssignment!.id}/revoke`)
    ...
    const stale = await request(app).post(`/api/mesaops/v1/role-assignments/${created.body.id}/revoke`)
    ...
    const revoked = await request(app).post(`/api/mesaops/v1/role-assignments/${created.body.id}/revoke`)
    ...
    const revokeReplay = await request(app).post(`/api/mesaops/v1/role-assignments/${created.body.id}/revoke`)
      .set('Idempotency-Key', revokeKey)
      .send({ expectedVersion: 0, reason: 'Plant assignment ended' });
    ...
    const reassigned = await request(app).patch(`/api/mesaops/v1/employees/${employee.body.id}`).send({ roleId: await roleId('Operator') });
    expect(reassigned.status).toBe(200);
    const deleteScopedRole = await request(app).delete(`/api/mesaops/v1/roles/${role.body.id}`);
  ...
  it('administers immutable MesaOps statutory rules with exact access, idempotency, CAS and maker-checker', async () => {
    ...
    const denied = await request(app).get('/api/mesaops/v1/admin/statutory-rule-profiles').set('x-dev-user', 'EMP-007');
    expect(denied.status).toBe(403);
    expect((await request(app).post('/api/mesaops/v1/admin/statutory-rule-profiles').set('x-dev-user', 'EMP-002').send(body)).status).toBe(400);
    ...
    const replay = await request(app).post('/api/mesaops/v1/admin/statutory-rule-profiles')
      .set('x-dev-user', 'EMP-002')
      .set('Idempotency-Key', key)
      .send(body);
    expect(replay.status).toBe(201);
    expect(replay.body.id).toBe(created.body.id);
    const conflict = await request(app).post('/api/mesaops/v1/admin/statutory-rule-profiles')
      .set('x-dev-user', 'EMP-002')
      .set('Idempotency-Key', key)
      .send({ ...body, version: `${body.version}-changed` });
    expect(conflict.body.error.code).toBe('idempotency_conflict');

    const selfApproval = await request(app).post(`/api/mesaops/v1/admin/statutory-rule-profiles/${created.body.id}/approve`)
      .set('x-dev-user', 'EMP-002')
    ...
    const stale = await request(app).post(`/api/mesaops/v1/admin/statutory-rule-profiles/${created.body.id}/approve`)
    ...
    const listed = await request(app).get('/api/mesaops/v1/admin/statutory-rule-profiles').set('x-dev-user', 'EMP-020');
  ...
  async function roleId(name: string): Promise<string> {
    const roles = (await request(app).get('/api/mesaops/v1/roles')).body as Array<{ id: string; name: string }>;
