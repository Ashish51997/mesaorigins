...
adminRouter.get('/role-assignments', requirePermission(ACL),
  ah(() => svc.listMesaOpsRoleAssignments()));
adminRouter.post('/role-assignments', requirePermission(ACL), validateBody(mesaOpsRoleAssignmentCreateSchema),
  ah(async (req, res) => {
    res.status(201);
    return svc.createMesaOpsRoleAssignment(req.body, idempotencyKey(req));
  }));
adminRouter.post('/role-assignments/:id/revoke', requirePermission(ACL), validateBody(mesaOpsRoleAssignmentRevokeSchema),
...
adminRouter.get('/admin/statutory-rule-profiles', requirePermission('action:mesaops.statutory_rule_profile.view'),
  ah(() => statutoryProfiles.listMesaOpsStatutoryRuleProfiles()));
adminRouter.get('/admin/statutory-rule-profiles/:id', requirePermission('action:mesaops.statutory_rule_profile.view'),
  ah((req) => statutoryProfiles.getMesaOpsStatutoryRuleProfile(req.params.id)));
adminRouter.post('/admin/statutory-rule-profiles', requirePermission('action:mesaops.statutory_rule_profile.create'),
...
adminRouter.post('/admin/statutory-rule-profiles/:id/approve', requirePermission('action:mesaops.statutory_rule_profile.approve'),
