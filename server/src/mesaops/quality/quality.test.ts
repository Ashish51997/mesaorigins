...
async function submitLogbookWithLot(machineCode: string, day: string, lot: string): Promise<void> {
  const c = await request(app).post('/api/mesaops/v1/customers').send({ name: `Q ${uniq()}` });
  const inq = await request(app).post('/api/mesaops/v1/inquiries').send({ customerId: c.body.id, product: 'RPVC roll', quantity: 1000, expectedDeliveryDate: '2026-10-01' });
  ...
  const machine = await request(app).post('/api/mesaops/v1/machines').send({ code: `${machineCode}${uniq()}`.slice(0, 16), line: 'Quality queue test', family: 'PVC' });
  const machineId = machine.body.id as string;
  const plan = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', idem()).send({ salesOrderId: ord.body.id, expectedOrderVersion: 0, machineId, shift: 'D', scheduledStartDate: `${day}T08:00:00`, supervisor: 'Nandlal', drawingNo: 'DRW-1', formulaNo: 'RF03 · Rev 2', moldNo: 'MLD-1', productName: 'RPVC' });
  const lb = await request(app).post('/api/mesaops/v1/logbooks').send({ productionPlanId: plan.body.id });
  await request(app).patch(`/api/mesaops/v1/logbooks/${lb.body.id}`).send({ operatorSignature: 'Nandlal', supervisorSignature: 'Nandlal', traceabilityRows: [{ lotNumber: lot, colour: 'Black', code: 'C1', winderPackedBy: 'x' }] });
...
describe('quality slice', () => {
  it('queues a packed roll, passes it, and books finished-goods stock', async () => {
    ...
    const queue = (await request(app).get('/api/mesaops/v1/quality/queue')).body as Array<{ lotNumber: string }>;
    expect(queue.some((q) => q.lotNumber === lot)).toBe(true);

    const insp = await request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: lot, decision: 'pass', weight: 24.5 });
    ...
    // It left the queue…
    const queue2 = (await request(app).get('/api/mesaops/v1/quality/queue')).body as Array<{ lotNumber: string }>;
    ...
    const dupe = await request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: lot, decision: 'pass', weight: 10 });
  ...
  it('a hold does not book stock', async () => {
    ...
    const held = await request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: lot, decision: 'hold', weight: 20, remarks: 'dimension drift' });
  ...
  it('serializes concurrent decisions so one lot creates one inspection and at most one FG receipt', async () => {
    ...
    const decisions = await Promise.all([
      request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: lot, decision: 'pass', weight: 12.5 }),
      request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: lot, decision: 'pass', weight: 12.5 }),
    ]);
  ...
  it('requires a positive inspected quantity', async () => {
    ...
    const zero = await request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: lot, decision: 'pass', weight: 0 });
  ...
  it('rejects an unknown lot (422)', async () => {
    const r = await request(app).post('/api/mesaops/v1/quality/inspections').send({ lotNumber: 'not-a-real-lot', decision: 'pass', weight: 5 });
    expect(r.status).toBe(422);
  });

  it('denies a Sales Exec from the roll queue (403)', async () => {
    const r = await request(app).get('/api/mesaops/v1/quality/queue').set('x-dev-user', 'EMP-003');
