...
describe('sales slice — validation & authz', () => {
  it('rejects a customer with no name (422)', async () => {
    const r = await request(app).post('/api/mesaops/v1/customers').send({ gstNumber: 'X' });
    expect(r.status).toBe(422);
  });

  it('enforces GST uniqueness within the tenant (409)', async () => {
    const gst = `GST-${uniq()}`;
    const a = await request(app).post('/api/mesaops/v1/customers').send({ name: 'Dup A', gstNumber: gst });
    expect(a.status).toBe(201);
    const b = await request(app).post('/api/mesaops/v1/customers').send({ name: 'Dup B', gstNumber: gst });
    expect(b.status).toBe(409);
  });

  it('denies an Operator from confirming an order (403)', async () => {
    const r = await request(app).post('/api/mesaops/v1/orders').set('x-dev-user', 'EMP-007').send({ inquiryId: 'x' });
    expect(r.status).toBe(403);
  });

  it('scopes the customer list to the tenant', async () => {
    const r = await request(app).get('/api/mesaops/v1/customers');
  ...
  it('hides and cannot cancel a legacy sales order assigned to another plant', async () => {
    ...
    try {
      const orders = await scoped.get('/api/mesaops/v1/orders');
      expect(orders.status).toBe(200);
      expect(orders.body.some((order: { id: string }) => order.id === fixture.order.id)).toBe(false);
      const cancel = await scoped.post(`/api/mesaops/v1/orders/${fixture.order.id}/cancel`);
...
describe('sales slice — lifecycle', () => {
  it('customer → inquiry → quote → order → cancel, with the guards', async () => {
    const cust = await request(app).post('/api/mesaops/v1/customers').send({ name: `Nova ${uniq()}`, gstNumber: `G-${uniq()}` });
    expect(cust.status).toBe(201);
    const customerId = cust.body.id as string;

    const inq = await request(app).post('/api/mesaops/v1/inquiries').send({
    ...
    const early = await request(app).post('/api/mesaops/v1/orders').send({ inquiryId });
    expect(early.status).toBe(409);

    const quote = await request(app).post(`/api/mesaops/v1/inquiries/${inquiryId}/quote`).send({ quotationPrice: 42 });
    expect(quote.status).toBe(200);
    expect(quote.body.status).toBe('quotation');
    expect(quote.body.quotationPrice).toBe(42);

    const order = await request(app).post('/api/mesaops/v1/orders').send({ inquiryId, priority: 'high' });
    ...
    const dupe = await request(app).post('/api/mesaops/v1/orders').send({ inquiryId });
    expect(dupe.status).toBe(409);

    // Cancel returns the inquiry to the quotation queue.
    const cancel = await request(app).post(`/api/mesaops/v1/orders/${orderId}/cancel`);
    expect(cancel.status).toBe(200);
    const inquiries = await request(app).get('/api/mesaops/v1/inquiries');
  ...
  it('rejects an inquiry with zero quantity (422)', async () => {
    const cust = await request(app).post('/api/mesaops/v1/customers').send({ name: `Zero ${uniq()}` });
    const r = await request(app).post('/api/mesaops/v1/inquiries').send({
