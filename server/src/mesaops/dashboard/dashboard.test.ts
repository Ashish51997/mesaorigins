...
describe('dashboard summary', () => {
  it('returns real tenant-scoped KPI aggregates', async () => {
    const r = await request(app).get('/api/mesaops/v1/summary');
  ...
  it('is reachable by every role (dashboard is universal)', async () => {
    ...
    const r = await request(app).get('/api/mesaops/v1/summary').set('x-dev-user', 'EMP-007');
    expect(r.status).toBe(200);
  });
});
...
describe('management overview', () => {
  it('returns the MD overview shape for Managing Director', async () => {
    const r = await request(app).get('/api/mesaops/v1/management/overview').set('x-dev-user', 'EMP-001');
    expect(r.status).toBe(200);
  ...
  it('forbids operators without management_dashboard', async () => {
    const r = await request(app).get('/api/mesaops/v1/management/overview').set('x-dev-user', 'EMP-007');
