...
describe('maintenance slice', () => {
  it('lists the tenant machine registry', async () => {
    const r = await request(app).get('/api/mesaops/v1/machines');
  ...
  it('registers a new machine and rejects duplicate codes', async () => {
    ...
    const add = await request(app).post('/api/mesaops/v1/machines').send({
    ...
    const dup = await request(app).post('/api/mesaops/v1/machines').send({
  ...
  it('adds a maintenance task for a machine, then completes it', async () => {
    ...
    const add = await request(app).post('/api/mesaops/v1/maintenance').send({
    ...
    const done = await request(app).post(`/api/mesaops/v1/maintenance/${add.body.id}/complete`);
    expect(done.status).toBe(200);
    expect(done.body.status).toBe('completed');
  });

  it('requires a machine (422 for missing, 422 for unknown)', async () => {
    const missing = await request(app).post('/api/mesaops/v1/maintenance').send({ taskName: 'x', dueDate: '2026-09-01' });
    expect(missing.status).toBe(422);
    const unknown = await request(app).post('/api/mesaops/v1/maintenance').send({ machineId: 'does-not-exist', taskName: 'x', dueDate: '2026-09-01' });
    expect(unknown.status).toBe(422);
  });

  it('denies a Sales Exec from the maintenance schedule (403)', async () => {
    const r = await request(app).get('/api/mesaops/v1/maintenance').set('x-dev-user', 'EMP-003');
