...
async function pendingOrders() {
  return (await request(app).get('/api/mesaops/v1/planning/orders')).body as Array<{
...
async function createPlanningMachine(plantCode: string, prefix: string) {
  ...
  const response = await request(app).post('/api/mesaops/v1/machines').send({
...
describe('planning slice', () => {
  it('lists only MesaOps operational demand awaiting planning', async () => {
    const r = await request(app).get('/api/mesaops/v1/planning/orders');
  ...
  it('schedules with shift header, seeds a draft logbook, prevents double-booking, and releases', async () => {
    ...
    const plan = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', planKey).send(planBody);
    ...
    const replayedPlan = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', planKey).send(planBody);
    expect(replayedPlan.status).toBe(201);
    expect(replayedPlan.body.id).toBe(plan.body.id);

    // Draft logbook carries the shift header.
    const lb = await request(app).get(`/api/mesaops/v1/logbooks/plan/${plan.body.id}`);
    ...
    const clash = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', idem('clash')).send({
    ...
    const patched = await request(app).patch(`/api/mesaops/v1/plans/${plan.body.id}`).set('Idempotency-Key', idem('patch')).send({ expectedVersion: plan.body.version, moldNo: 'MLD-99', drawingNo: 'DRW-99' });
    expect(patched.status).toBe(200);
    expect(patched.body.moldNo).toBe('MLD-99');
    const stalePatch = await request(app).patch(`/api/mesaops/v1/plans/${plan.body.id}`).set('Idempotency-Key', idem('stale-patch')).send({ expectedVersion: plan.body.version, moldNo: 'STALE' });
    expect(stalePatch.status).toBe(409);
    expect(stalePatch.body.error.code).toBe('version_conflict');
    const lb2 = await request(app).get(`/api/mesaops/v1/logbooks/plan/${plan.body.id}`);
    expect(lb2.body.moldNo).toBe('MLD-99');
    expect(lb2.body.drawingNo).toBe('DRW-99');

    const releaseKey = idem('release');
    const rel = await request(app).post(`/api/mesaops/v1/plans/${plan.body.id}/release`).set('Idempotency-Key', releaseKey).send({ expectedVersion: patched.body.version });
    expect(rel.status).toBe(200);
    const releaseReplay = await request(app).post(`/api/mesaops/v1/plans/${plan.body.id}/release`).set('Idempotency-Key', releaseKey).send({ expectedVersion: patched.body.version });
    ...
    expect((await request(app).get(`/api/mesaops/v1/logbooks/plan/${plan.body.id}`)).body).toBeNull();
  });
  ...
  it('rejects PATCH after the schedule start time', async () => {
    ...
    const plan = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', idem('past-plan')).send({
      operationalOrderId: order.id, plannedQuantity: order.remainingQuantity,
      expectedOrderVersion: order.rowVersion,
    ...
    expect(plan.status).toBe(201);
    const r = await request(app).patch(`/api/mesaops/v1/plans/${plan.body.id}`).set('Idempotency-Key', idem('past-patch')).send({ expectedVersion: plan.body.version, moldNo: 'X' });
    expect(r.status).toBe(409);
    expect(r.body.error.code).toBe('already_started');
    await request(app).post(`/api/mesaops/v1/plans/${plan.body.id}/release`).set('Idempotency-Key', idem('past-release')).send({ expectedVersion: plan.body.version });
  });

  it('requires shift header fields on create (422)', async () => {
    const { machine, order } = await createPlanningFixture('INVALID');
    const r = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', idem('invalid-plan')).send({
      operationalOrderId: order.id, plannedQuantity: order.remainingQuantity,
      expectedOrderVersion: order.rowVersion,
  ...
  it('rejects an unknown machine (422)', async () => {
    ...
    const r = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', idem('bad-machine')).send({
      operationalOrderId: order.id, plannedQuantity: order.remainingQuantity,
      expectedOrderVersion: order.rowVersion,
  ...
  it('denies an Operator from scheduling (403)', async () => {
    const r = await request(app).post('/api/mesaops/v1/plans').set('x-dev-user', 'EMP-007')
  ...
  it('keeps split-machine quantity and source state in the operational queue', async () => {
    ...
    const plan = await request(app).post('/api/mesaops/v1/plans').set('Idempotency-Key', idem('split-plan')).send({
    ...
    const released = await request(app).post(`/api/mesaops/v1/plans/${plan.body.id}/release`).set('Idempotency-Key', idem('split-release')).send({ expectedVersion: plan.body.version });
  ...
  it('accepts an immutable MesaERP snapshot once and marks later source changes stale', async () => {
    ...
    const accepted = await request(app).post('/api/mesaops/v1/operational-orders/handoffs/mesaerp').set(signMesaErpToOpsHandoff('org-demo', body)).send(body);
    expect(accepted.status).toBe(201);
    expect(accepted.body.status).toBe('accepted');
    expect(accepted.body.operationalOrder).toMatchObject({ sourceType: 'mesaerp', sourceLinkState: 'linked' });

    const replay = await request(app).post('/api/mesaops/v1/operational-orders/handoffs/mesaerp').set(signMesaErpToOpsHandoff('org-demo', body)).send(body);
    ...
    const changed = await request(app).post('/api/mesaops/v1/operational-orders/handoffs/mesaerp').set(signMesaErpToOpsHandoff('org-demo', changedBody)).send(changedBody);
  ...
  it('rejects a normal user attempting to forge a MesaERP source assertion', async () => {
    ...
    const response = await request(app).post('/api/mesaops/v1/operational-orders/handoffs/mesaerp').send(body);
  ...
  it('lets a planner accept only the immutable MesaERP event stored in the outbox', async () => {
    ...
    const inbox = await request(app).get('/api/mesaops/v1/operational-orders/handoffs/mesaerp');
    ...
    const accepted = await request(app)
      .post(`/api/mesaops/v1/operational-orders/handoffs/mesaerp/${body.eventId}/accept`)
  ...
  it('filters machine and planning access once an explicit MesaOps plant assignment exists', async () => {
    ...
    let machineId = '';
    try {
      const hidden = await scoped.get('/api/mesaops/v1/machines');
      expect(hidden.status).toBe(200);
      expect(hidden.body).toHaveLength(0);
      const created = await scoped.post('/api/mesaops/v1/machines').send({ code: `S${Date.now().toString(36)}`.slice(0, 16), plantCode, line: 'Scoped line', family: 'PVC' });
      expect(created.status).toBe(201);
      machineId = created.body.id;
      const visible = await scoped.get('/api/mesaops/v1/machines');
      expect(visible.body).toHaveLength(1);
      expect(visible.body[0].plantCode).toBe(plantCode);

      const plans = await scoped.get('/api/mesaops/v1/plans');
      expect(plans.body.some((plan: { id: string }) => plan.id === fixture.plan.id)).toBe(false);
      const tasks = await scoped.get('/api/mesaops/v1/logbook/tasks');
      expect(tasks.body.some((group: { machine: string }) => group.machine === fixture.machine.code)).toBe(false);
      expect((await scoped.get('/api/mesaops/v1/logbook/machine-hub').query({ machine: fixture.machine.code })).status).toBe(404);
      expect((await scoped.get('/api/mesaops/v1/logbook/resolve').query({ machine: fixture.machine.code })).status).toBe(404);
      expect((await scoped.get(`/api/mesaops/v1/logbooks/plan/${fixture.plan.id}`)).body).toBeNull();
      expect((await scoped.patch(`/api/mesaops/v1/logbooks/${fixture.logbook.id}`).send({ operatorSignature: 'forged' })).status).toBe(404);
      expect((await scoped.post(`/api/mesaops/v1/logbooks/${fixture.logbook.id}/submit`)).status).toBe(404);
      expect((await scoped.patch(`/api/mesaops/v1/plans/${fixture.plan.id}`)
        .set('Idempotency-Key', `hidden-plan-patch-${suffix}`)
        .send({ expectedVersion: fixture.plan.version, moldNo: 'forged' })).status).toBe(404);
      expect((await scoped.post(`/api/mesaops/v1/plans/${fixture.plan.id}/release`)
        .set('Idempotency-Key', `hidden-plan-release-${suffix}`)
        .send({ expectedVersion: fixture.plan.version })).status).toBe(404);

      const queue = await scoped.get('/api/mesaops/v1/quality/queue');
      expect(queue.body.some((row: { lotNumber: string }) => row.lotNumber === hiddenLot)).toBe(false);
      const inspections = await scoped.get('/api/mesaops/v1/quality/inspections');
      expect(inspections.body.some((row: { id: string }) => row.id === fixture.inspection.id)).toBe(false);
      expect((await scoped.post('/api/mesaops/v1/quality/inspections').send({ lotNumber: hiddenLot, decision: 'pass', weight: 10 })).status).toBe(422);

      const transactions = await scoped.get('/api/mesaops/v1/inventory/transactions');
      expect(transactions.body.some((row: { id: string }) => row.id === fixture.inventory.id)).toBe(false);
      const stock = await scoped.get('/api/mesaops/v1/inventory/stock');
      expect(stock.body.rawMaterials.some((row: { itemName: string }) => row.itemName === hiddenMaterial)).toBe(false);
      expect((await scoped.post('/api/mesaops/v1/inventory/issue').send({ itemName: hiddenMaterial, quantity: 1, unit: 'kg', machineId: fixture.machine.id })).status).toBe(422);

      const maintenance = await scoped.get('/api/mesaops/v1/maintenance');
      expect(maintenance.body.some((row: { id: string }) => row.id === fixture.maintenance.id)).toBe(false);
      expect((await scoped.post(`/api/mesaops/v1/maintenance/${fixture.maintenance.id}/complete`)).status).toBe(404);

      expect((await scoped.get('/api/mesaops/v1/complaints/batches')).body.some((row: { id: string }) => row.id === fixture.dispatch.id)).toBe(false);
      expect((await scoped.get('/api/mesaops/v1/complaints')).body.some((row: { id: string }) => row.id === fixture.complaint.id)).toBe(false);
      expect((await scoped.get('/api/mesaops/v1/capas')).body.some((row: { id: string }) => row.id === fixture.capa.id)).toBe(false);
      expect((await scoped.post('/api/mesaops/v1/complaints').send({ salesOrderId: fixture.order.id, severity: 'low', description: 'forged' })).status).toBe(422);
      expect((await scoped.patch(`/api/mesaops/v1/capas/${fixture.capa.id}`).send({ rootCause: 'forged' })).status).toBe(404);
      expect((await scoped.post(`/api/mesaops/v1/capas/${fixture.capa.id}/close`)).status).toBe(404);
      expect((await scoped.post(`/api/mesaops/v1/complaints/${fixture.complaint.id}/resolve`)).status).toBe(404);
      const overview = await scoped.get('/api/mesaops/v1/management/overview');
