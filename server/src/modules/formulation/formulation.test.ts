...
describe('formulation slice', () => {
  it('lists the seeded formulations with their revisions and lock state', async () => {
    const r = await request(app).get('/api/formulations');
  ...
  it('adds a formulation, then a new revision supersedes the prior active one', async () => {
    const code = `TF${uniq()}`;
    const first = await request(app).post('/api/formulations').send({
    ...
    const second = await request(app).post('/api/formulations').send({
    ...
    const rows = (await request(app).get('/api/formulations')).body as Formula[];
  ...
  it('edits a revision, and refuses to edit a locked one (409)', async () => {
    ...
    const ok = await request(app).patch(`/api/formulations/${editable.id}`)
    ...
    const blocked = await request(app).patch(`/api/formulations/${locked.id}`).send({ product: 'nope' });
    expect(blocked.status).toBe(409);
  });

  it('denies an Operator from the formulations board (403)', async () => {
    const r = await request(app).get('/api/formulations').set('x-dev-user', 'EMP-007');
