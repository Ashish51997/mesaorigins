...
function signingKey(): Buffer | null {
  const encoded = (process.env.MESADESK_ERP_OPS_HANDOFF_HMAC_KEY || '').trim();
...
export function signMesaErpToOpsHandoff(
  ...
  if (!key) throw new Error('MESADESK_ERP_OPS_HANDOFF_HMAC_KEY must be base64 and decode to at least 32 bytes.');
  const signature = createHmac('sha256', key).update(message(organizationId, timestamp, body)).digest('hex');
  return {
    'x-mesadesk-source-service': 'mesaerp',
    'x-mesadesk-timestamp': timestamp,
    'x-mesadesk-signature': signature,
  };
}
...
export const requireMesaErpHandoffSignature: RequestHandler = (req, res, next) => {
  ...
  const source = (req.header('x-mesadesk-source-service') || '').trim().toLowerCase();
  const timestamp = (req.header('x-mesadesk-timestamp') || '').trim();
  const suppliedHex = (req.header('x-mesadesk-signature') || '').trim().toLowerCase();
