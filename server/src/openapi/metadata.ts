...
export const ROUTE_DOCS: Record<string, RouteDoc> = {
  ...
  'POST /api/plans': {
    ...
    description: 'Moves the order to `planned`. One machine can hold only one plan per shift per day.',
    status: 201,
    responseDescription: 'The scheduled plan.',
    responseModel: 'ProductionPlan', responseIncludes: ['machine', 'salesOrder'],
    errors: [
      NOT_FOUND('order'),
