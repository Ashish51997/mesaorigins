 ...
 * guard — the value still saves). See docs/specs/spec-logbook.md.
 *
...
import { LogbookTemplate, MachineLogbook, HourlyInspectionRow, TraceabilityRow } from '@mesaops/types';
import { isInvalidNumber, isOutOfRange, sanitizeDecimal, sanitizeMeter, normalizeDate, normalizeTime, isInvalidDate, isInvalidTime, isInvalidMeter } from '@mesaops/lib/logbookValidation';
...
const CSS = `
.sheet-wrap{background:#F8FAFC;padding:16px;font-family:Roboto,system-ui,-apple-system,"Segoe UI",sans-serif;color:#0F172A}
.toolbar{max-width:920px;margin:0 auto 10px;display:flex;justify-content:flex-end}
.print-btn{border:1px solid #1E40AF;background:#1E40AF;color:#fff;border-radius:8px;padding:10px 16px;font-size:14px;font-weight:500;cursor:pointer;min-height:44px}
.sheet{max-width:920px;margin:0 auto;background:#fff;border:1px solid #E2E8F0;border-radius:12px;padding:10px 12px 14px;font-size:13px}
.sheet *{box-sizing:border-box}

.cin{border:0;background:transparent;font-family:inherit;font-size:14px;width:100%;min-height:28px;padding:4px 6px;outline:none;color:#0F172A}
.cin:focus{background:#E0F2FE}
.cin.oor{background:#FEF3C7;color:#92400E;font-weight:700}
.cin.active{background:#DBEAFE;box-shadow:0 0 0 2px #1E40AF inset;border-radius:4px}
.cin[readonly]{cursor:default}
.cin-pick{min-width:8.5rem;font-size:13px;min-height:30px}
.sign-lbl{font-size:10px;font-weight:700;color:#64748B;margin-bottom:2px}
.sign .cin{border-bottom:1px solid #94A3B8;min-height:28px}

/* permissible-value hint on measurement cells */
.rc{position:relative;display:block}
.hint{position:absolute;z-index:30;top:100%;left:0;margin-top:2px;background:#1E40AF;color:#fff;font-size:10px;font-weight:700;padding:3px 7px;border-radius:6px;white-space:nowrap}
.hint::before{content:"";position:absolute;top:-4px;left:8px;border-left:4px solid transparent;border-right:4px solid transparent;border-bottom:4px solid #1E40AF}

/* click-to-open dropdown */
.drop{position:relative;display:inline-block}
.drop-btn{display:inline-flex;align-items:center;justify-content:space-between;gap:4px;width:100%;min-height:28px;border:0;background:transparent;font-family:inherit;font-size:14px;cursor:pointer;padding:4px 6px;color:#0F172A;border-radius:4px}
.drop-btn:hover{background:#E0F2FE}
.drop-btn.active{background:#DBEAFE;box-shadow:0 0 0 2px #1E40AF inset}
.drop-btn:disabled{cursor:default}
.drop-val{font-weight:600}
.drop-ph{color:#94A3B8}
.drop-btn svg{flex:0 0 auto;color:#64748B}
.drop-back{position:fixed;inset:0;z-index:40}
.drop-menu{position:absolute;z-index:41;top:100%;left:0;margin:2px 0 0;padding:4px;list-style:none;min-width:130px;background:#fff;border:1px solid #E2E8F0;border-radius:8px}
.drop-menu li{margin:0}
.drop-menu li button{display:flex;align-items:center;justify-content:space-between;width:100%;text-align:left;border:0;background:transparent;font-family:inherit;font-size:13px;padding:8px 10px;border-radius:6px;cursor:pointer;color:#0F172A;min-height:44px}
.drop-menu li button:hover{background:#E0F2FE}
.drop-menu li button.on{background:#1E40AF;color:#fff;font-weight:600}

.top{display:flex;align-items:stretch;border:1px solid #E2E8F0;border-bottom:0;border-radius:8px 8px 0 0;overflow:hidden}
.brand{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:4px 10px;border-right:1px solid #E2E8F0;min-width:110px;background:#F8FAFC}
.logo{font-weight:800;font-size:13px;line-height:1;text-align:center;color:#0F172A}
.loc{font-size:8px;letter-spacing:1px;margin-top:2px;color:#64748B}
.title{flex:1;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;letter-spacing:1px;border-right:1px solid #E2E8F0;color:#0F172A}
.docbox{border-collapse:collapse;font-size:11px}
.docbox td{border:1px solid #E2E8F0;padding:2px 6px;white-space:nowrap;color:#475569}
.docbox .dc{font-weight:700;color:#0F172A}

.idrow{display:flex;flex-wrap:wrap;gap:0;border:1px solid #E2E8F0;align-items:center;margin-bottom:14px;color:#475569}
.idrow>span{display:flex;align-items:center;gap:4px;padding:4px 8px;border-right:1px solid #E2E8F0;font-weight:600}
...
.band{background:#E0F2FE;border:1px solid #E2E8F0;padding:3px 8px;font-weight:800;font-size:13px;cursor:pointer;color:#0F172A}
.band.active{background:#DBEAFE;box-shadow:inset 3px 0 0 #1E40AF}
.band .bnum{margin-right:2px}
.band .bsub{font-weight:600;font-size:11px;margin-left:12px;color:#475569}
.band .bright{float:right;font-weight:600;font-size:11px;color:#475569}

.lbgrid{width:100%;border-collapse:collapse;border:1px solid #E2E8F0;border-top:0}
.lbgrid th,.lbgrid td{border:1px solid #E2E8F0;padding:3px 4px;text-align:center;vertical-align:middle}
.lbgrid th{background:#F8FAFC;font-size:10px;font-weight:700;line-height:1.1;color:#475569}
.lbgrid th .sp{font-weight:400;font-size:9px;color:#64748B}
.lbgrid td{height:30px;color:#475569}
.lbgrid .lbl{background:#F1F5F9;font-weight:600;text-align:left;white-space:nowrap;font-size:11px;color:#475569}
.lbgrid.tight td{height:28px}
.lbgrid .tcell{font-weight:700;background:#F1F5F9;white-space:nowrap;color:#0F172A}

/* section 2 layout */
.insp{display:flex;align-items:flex-start;border:1px solid #E2E8F0;border-top:0}
.insp-left{width:300px;flex:0 0 300px;border-right:1px solid #E2E8F0}
.dimbox{border-bottom:1px solid #E2E8F0;padding:4px 6px;text-align:center}
.dim-t,.dim-b{font-size:10px;font-weight:600;color:#475569}
.dimsvg{width:150px;height:56px}
.coil-hd{padding:4px 6px;font-size:10px;border-bottom:1px solid #E2E8F0;line-height:1.3;color:#475569}
.coil-grid{display:grid;grid-template-columns:1fr 1fr}
.coil-cell{display:flex;align-items:center;border-bottom:1px solid #E2E8F0;border-right:1px solid #E2E8F0;padding:0 2px}
.coil-cell .cn{font-size:10px;font-weight:700;width:24px;flex:0 0 24px;color:#0F172A}
.insp-grid{border:0;flex:1}
.insp-grid td{height:32px}

/* section 3 */
.trace{display:flex;gap:0;border:1px solid #E2E8F0;border-top:0}
.trace-grid{border:0;border-right:1px solid #E2E8F0}
.trace-grid:last-child{border-right:0}
.trace-grid td{height:28px;text-align:left}

/* section 4 */
.rej{border:1px solid #E2E8F0;border-top:0;padding:5px 8px;display:flex;flex-wrap:wrap;align-items:center;gap:4px 16px}
.rej-hd{font-weight:800;color:#0F172A}
.rej-item{display:flex;align-items:center;gap:4px;font-size:11px;border-bottom:1px solid #E2E8F0;color:#475569}
.notes{border:1px solid #E2E8F0;border-top:0;padding:5px 8px;font-size:10px;line-height:1.5;color:#475569}
.sign{display:flex;justify-content:space-between;border:1px solid #E2E8F0;border-top:0;padding:26px 20px 8px;font-weight:700;font-size:12px;color:#0F172A}

@media print{
  .sheet-wrap{background:#fff;padding:0}
  .toolbar{display:none}
  .sheet{border:1.5px solid #000;max-width:none;border-radius:0}
...
@media(max-width:700px){
  .insp{flex-direction:column}.insp-left{width:100%;flex:1 1 auto;border-right:0;border-bottom:1px solid #E2E8F0}
  .trace{flex-direction:column}.trace-grid{border-right:0;border-bottom:1px solid #E2E8F0}
