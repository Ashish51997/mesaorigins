...
import { useQualityQueue, useQualityInspections, useCreateInspection, type ApiQueueItem } from '@mesaops/lib/queries/quality';
    ...
    <ResponsiveOverlay open onClose={onClose} title={item.lotNumber}>
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button disabled={!canPass || create.isPending} title={canPass ? undefined : 'No access — ask your administrator'} onClick={() => decide('pass')} className={`h-14 rounded-lg bg-emerald-600 text-white font-bold inline-flex items-center justify-center gap-1 ${canPass && !create.isPending ? '' : 'opacity-50 cursor-not-allowed'}`}><CheckCircle2 className="w-5 h-5" /> PASS</button>
            <button disabled={!canHold || create.isPending} title={canHold ? undefined : 'No access — ask your administrator'} onClick={() => decide('hold')} className={`h-14 rounded-lg bg-amber-500 text-white font-bold inline-flex items-center justify-center gap-1 ${canHold && !create.isPending ? '' : 'opacity-50 cursor-not-allowed'}`}><PauseCircle className="w-5 h-5" /> HOLD</button>
            <button disabled={create.isPending} onClick={() => decide('fail')} className="h-14 rounded-lg bg-rose-600 text-white font-bold inline-flex items-center justify-center gap-1 disabled:opacity-50"><XCircle className="w-5 h-5" /> FAIL</button>
          </div>
