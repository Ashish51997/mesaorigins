...
const typeChip = (t: string) => <StatusBadge tone="neutral">{t}</StatusBadge>;
...
const STATUS: Record<string, { tone: StatusTone; word: string }> = {
  running: { tone: 'success', word: 'Running' },
  attention: { tone: 'warn', word: 'Needs a look' },
  stopped: { tone: 'error', word: 'Stopped' },
};
    ...
    <div className="space-y-4" data-testid="machine-qr-panel">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <button
          ...
          onClick={onDownload}
          disabled={busy}
          className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 text-sm font-bold text-white hover:bg-indigo-500 disabled:opacity-40"
        >
        ...
        <button
          type="button"
          onClick={onCopy}
          className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-slate-50"
        >
          <Copy className="h-4 w-4" /> Copy link
        </button>
      </div>
      {onDone && (
        <button type="button" onClick={onDone} className="w-full min-h-[44px] rounded-lg border border-slate-200 text-sm font-bold text-slate-600">
          Done
        </button>
      )}
    </div>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'status', header: 'Status', cell: (m) => {
            ...
            return <StatusBadge tone={s.tone}>{s.word}</StatusBadge>;
          } },
    ...
    <ResponsiveOverlay open onClose={onClose} title="Add a machine">
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-1">
          <button onClick={onClose} className="min-h-[44px] px-4 rounded-lg border border-slate-200 text-sm font-bold text-slate-600">Cancel</button>
          <button onClick={submit} disabled={!valid || create.isPending} className="min-h-[44px] px-5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-bold inline-flex items-center gap-1.5"><Plus className="w-4 h-4" /> Add machine</button>
        </div>
      </div>
    </ResponsiveOverlay>
  );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <button onClick={() => setShowAdd(true)} disabled={machines.length === 0} title={machines.length === 0 ? 'No machines in this plant yet' : undefined} className="inline-flex items-center gap-1.5 min-h-[44px] px-4 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-bold cursor-pointer shrink-0">
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'due', header: 'Due', cell: (t) => {
            ...
            return (
              <StatusBadge tone={overdue ? 'error' : 'neutral'}>
  ...
  return (
    <ResponsiveOverlay open onClose={onClose} title="Add a maintenance task">
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-1">
          <button onClick={onClose} className="min-h-[44px] px-4 rounded-lg border border-slate-200 text-sm font-bold text-slate-600">Cancel</button>
          <button onClick={submit} disabled={!valid || addTask.isPending} className="min-h-[44px] px-5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-bold inline-flex items-center gap-1.5"><Plus className="w-4 h-4" /> Schedule task</button>
        </div>
      </div>
    </ResponsiveOverlay>
  );
