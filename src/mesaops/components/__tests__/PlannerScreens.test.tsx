...
describe('Planner — orders to plan (API)', () => {
  it('creates an independent MesaOps operational order before machine planning', async () => {
    get.mockImplementation((path: string) => path === '/mesaops/v1/planning/orders' ? Promise.resolve([]) : Promise.resolve([]));
  ...
  it('lists an operational order and schedules a valid quantity split with required MesaOps fields', async () => {
    get.mockImplementation((path: string) => {
      if (path === '/mesaops/v1/planning/orders') return Promise.resolve([orderRow]);
      if (path === '/mesaops/v1/machines') return Promise.resolve([machine]);
      if (path === '/mesaops/v1/planning/operators') return Promise.resolve([{ id: 'op1', employeeCode: 'EMP-007', role: 'Operator', user: { name: 'Nandlal' } }]);
      if (path === '/mesaops/v1/directory') return Promise.resolve([{ id: 'sup1', name: 'Supervisor One', email: 'supervisor@example.test', role: 'Supervisor', employeeCode: 'EMP-002', department: 'Production' }]);
      if (path === '/mesaops/v1/logbook/formulas') return Promise.resolve([{ id: 'f1', code: 'RF03', rev: 2, product: 'SPVC Sheathing' }]);
      if (path === '/mesaops/v1/logbook/templates') return Promise.resolve([{ id: 'tp1', docNo: 'QR/MFG/013', layout: 'coil', productName: 'SPVC Sheathing' }]);
    ...
    await waitFor(() => expect(postIdempotent).toHaveBeenCalledWith('/mesaops/v1/plans', expect.objectContaining({
  ...
  it('still renders the legacy sales-order-shaped response', async () => {
    get.mockImplementation((path: string) => path === '/mesaops/v1/planning/orders'
...
describe('Planner — plan board (API)', () => {
  it('lists production plans and releases one', async () => {
    ...
    await waitFor(() => expect(postIdempotent).toHaveBeenCalledWith('/mesaops/v1/plans/p1/release', { expectedVersion: 4 }, expect.stringMatching(/^production-plan-release:/)));
  });
});
...
describe('Planner — formulations (API)', () => {
  ...
  it('adds a formulation via the API', async () => {
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/mesaops/v1/formulations', expect.objectContaining({
