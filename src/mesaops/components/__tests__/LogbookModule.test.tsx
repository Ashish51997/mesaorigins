...
import { initialLogbookTemplates } from '@mesaops/mockData';
...
function setupApi(plans: unknown[] = [apiPlan], logbook: Record<string, unknown> = blankLogbook()) {
  get.mockImplementation((path: string) => {
    if (path === '/mesaops/v1/logbook/templates') return Promise.resolve([template]);
    if (path === '/mesaops/v1/logbook/plans') return Promise.resolve(plans);
    if (path === '/mesaops/v1/logbook/formulas') return Promise.resolve([{ id: 'f1', code: 'RF03', rev: 2, label: 'RF03 · Rev 2' }]);
    return Promise.resolve([]);
  });
  post.mockImplementation((path: string) => {
    if (path === '/mesaops/v1/logbooks') return Promise.resolve(logbook);
...
describe('LogbookModule (operator)', () => {
  ...
  it('opens the logbook for the scheduled plan and renders guided preview + wizard', async () => {
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/mesaops/v1/logbooks', { productionPlanId: PLAN_ID }));
  });
  ...
  it('submitting saves progress without locking (empty fields allowed)', async () => {
    ...
    expect(post).not.toHaveBeenCalledWith('/mesaops/v1/logbooks/lb-1/submit');
  });
  ...
  it('closing a filled sheet saves then submits via the API', async () => {
    ...
    await waitFor(() => expect(panelInput('Machine No').value).toBe('M08'));
    fireEvent.click(screen.getByText('Close'));
    await waitFor(() => expect(post).toHaveBeenCalledWith('/mesaops/v1/logbooks/lb-1/submit'));
