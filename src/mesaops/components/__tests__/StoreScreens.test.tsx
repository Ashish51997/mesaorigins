...
describe('Store — receive (API)', () => {
  it('receives raw material via the API', async () => {
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/mesaops/v1/inventory/receive', expect.objectContaining({ itemName: 'CaCO3 filler', quantity: 500 })));
  });
});
...
describe('Store — issue (API)', () => {
  it('issues RM to a machine via the API', async () => {
    get.mockImplementation((path: string) => {
      if (path === '/mesaops/v1/inventory/stock') return Promise.resolve(stock);
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/mesaops/v1/inventory/issue', expect.objectContaining({ itemName: 'RPVC resin', quantity: 200, machineId: 'm1' })));
  });
});
