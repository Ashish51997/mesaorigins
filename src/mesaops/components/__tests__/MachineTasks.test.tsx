...
beforeEach(() => {
  ...
  get.mockImplementation((path: string) => {
    if (path === '/mesaops/v1/logbook/tasks') return Promise.resolve([]);
...
describe('MachineTasks QR → hub', () => {
  it('opens the machine hub when a QR code is provided', async () => {
    get.mockImplementation((path: string) => {
      if (path === '/mesaops/v1/logbook/tasks') return Promise.resolve([]);
      if (path.startsWith('/mesaops/v1/logbook/machine-hub')) return Promise.resolve(hubPayload());
      return Promise.resolve({});
    });
  ...
  it('shows not-found when hub returns 404', async () => {
    get.mockImplementation((path: string) => {
      if (path === '/mesaops/v1/logbook/tasks') return Promise.resolve([]);
      if (path.startsWith('/mesaops/v1/logbook/machine-hub')) {
