...
export function useQualityQueue() {
  return useQuery({ queryKey: keys.queue, queryFn: () => api.get<ApiQueueItem[]>(mesaOpsPath('/quality/queue')) });
}
export function useQualityInspections() {
  return useQuery({ queryKey: keys.inspections, queryFn: () => api.get<ApiInspection[]>(mesaOpsPath('/quality/inspections')) });
}
export function useCreateInspection() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Record<string, unknown>) => api.post<ApiInspection>(mesaOpsPath('/quality/inspections'), body),
    onSuccess: () => {
