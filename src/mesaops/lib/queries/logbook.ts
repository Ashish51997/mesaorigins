...
export function useLogbookTemplates() {
  return useQuery({ queryKey: keys.templates, queryFn: () => api.get<LogbookTemplate[]>(mesaOpsPath('/logbook/templates')) });
}
export function useLogbookFormulas() {
  return useQuery({ queryKey: keys.formulas, queryFn: () => api.get<ApiLogbookFormula[]>(mesaOpsPath('/logbook/formulas')) });
}
export function useLogbookPlans() {
  return useQuery({ queryKey: keys.plans, queryFn: () => api.get<ApiPlanToLog[]>(mesaOpsPath('/logbook/plans')) });
}
...
export function useLogbookTasks() {
  return useQuery({ queryKey: ['logbook', 'tasks'], queryFn: () => api.get<ApiMachineTaskGroup[]>(mesaOpsPath('/logbook/tasks')) });
}
...
export function useResolveMachineLogbook(machineCode: string | null | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () => api.get<ApiResolveMachineLogbook>(mesaOpsPath(`/logbook/resolve?machine=${encodeURIComponent(code)}`)),
    retry: false,
  });
}
...
export function useMachineHub(machineCode: string | null | undefined) {
  ...
  return useQuery({
    ...
    enabled: code.length > 0,
    queryFn: () => api.get<ApiMachineHub>(mesaOpsPath(`/logbook/machine-hub?machine=${encodeURIComponent(code)}`)),
    retry: false,
  });
}
...
export function useLogbookLedger(range?: { from?: string; to?: string }) {
  ...
  return useQuery({
    ...
    queryFn: () => api.get<ApiLogbookLedger>(mesaOpsPath(`/logbook/ledger${q ? `?${q}` : ''}`)),
  });
}

export function useOpenLogbook() {
  return useMutation({ mutationFn: (productionPlanId: string) => api.post<MachineLogbook>(mesaOpsPath('/logbooks'), { productionPlanId }) });
}
export function useSaveLogbook() {
  return useMutation({ mutationFn: ({ id, patch }: { id: string; patch: Partial<MachineLogbook> }) => api.patch<MachineLogbook>(mesaOpsPath(`/logbooks/${id}`), patch) });
}
export function useSubmitLogbook() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.post<MachineLogbook>(mesaOpsPath(`/logbooks/${id}/submit`)),
    onSuccess: () => {
