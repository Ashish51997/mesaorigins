...
export function useMyPermissions(identity: string) {
  return useQuery({
    ...
    queryFn: () => api.get<{ isAdmin: boolean; screens: string[] }>(mesaOpsPath('/me/permissions')),
  });
}
// Roster for the login picker / role switcher.
export function useDirectory(enabled = true) {
  return useQuery({ queryKey: ['directory'], queryFn: () => api.get<ApiDirectoryEntry[]>(mesaOpsPath('/directory')), enabled });
}

export function useEmployees() {
  return useQuery({ queryKey: keys.employees, queryFn: () => api.get<ApiEmployee[]>(mesaOpsPath('/employees')) });
}
export function useRoles() {
  return useQuery({ queryKey: keys.roles, queryFn: () => api.get<ApiRole[]>(mesaOpsPath('/roles')) });
}
export function useScreenCatalog() {
  return useQuery({ queryKey: keys.screens, queryFn: () => api.get<{ screens: string[] }>(mesaOpsPath('/screens')).then((r) => r.screens) });
}
export function useEmployeeGrants(membershipId: string | null) {
  return useQuery({
    queryKey: keys.grants(membershipId ?? ''),
    queryFn: () => api.get<ApiGrant[]>(mesaOpsPath(`/employees/${membershipId}/grants`)),
...
export function useMesaOpsRoleAssignments() {
  return useQuery({
    ...
    queryFn: () => api.get<ApiMesaOpsRoleAssignment[]>(mesaOpsPath('/role-assignments')),
  });
}
...
export function useCreateEmployee() {
  ...
  return useMutation({
    mutationFn: (body: { name: string; email: string; roleId: string; department?: string; shift?: string; status?: string }) =>
      api.post<ApiEmployee>(mesaOpsPath('/employees'), body),
...
export function useUpdateEmployee() {
  ...
  return useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<{ roleId: string; status: string; department: string; shift: string }> }) =>
      api.patch<ApiEmployee>(mesaOpsPath(`/employees/${id}`), patch),
...
export function useCreateRole() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: { name: string; screens: string[] }) => api.post<ApiRole>(mesaOpsPath('/roles'), body),
...
export function useUpdateRole() {
  ...
  return useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<{ name: string; screens: string[] }> }) =>
      api.patch<ApiRole>(mesaOpsPath(`/roles/${id}`), patch),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.roles }),
  });
}
export function useDeleteRole() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.del<{ ok: boolean }>(mesaOpsPath(`/roles/${id}`)),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.roles }),
  });
}
export function useSetGrants() {
  ...
  return useMutation({
    mutationFn: ({ id, grants }: { id: string; grants: { screen: string; state: 'on' | 'off' }[] }) =>
      api.put<ApiGrant[]>(mesaOpsPath(`/employees/${id}/grants`), { grants }),
...
export function useCreateMesaOpsRoleAssignment() {
  ...
  return useMutation({
    mutationFn: ({ input, requestKey }: {
      ...
      requestKey: string;
    }) => api.postIdempotent<ApiMesaOpsRoleAssignment>(mesaOpsPath('/role-assignments'), input, requestKey),
...
export function useRevokeMesaOpsRoleAssignment() {
  ...
  return useMutation({
    mutationFn: ({ assignmentId, expectedVersion, reason, requestKey }: {
      ...
      mesaOpsPath(`/role-assignments/${assignmentId}/revoke`),
