...
describe('Sales — customers (API-backed)', () => {
  ...
  it('creates a customer via the API', async () => {
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/customers', expect.objectContaining({ name: 'Nova Polymers' })));
  });
});
...
describe('Sales — order (API-backed)', () => {
  it('confirms a quoted inquiry via the API', async () => {
    get.mockImplementation((path: string) => {
      if (path === '/customers') return Promise.resolve(customers);
      if (path === '/inquiries') return Promise.resolve([{ id: 'i1', inquiryNumber: 'INQ-1', customerId: 'c1', product: 'RPVC 20mm', quantity: 1000, status: 'quotation', expectedDeliveryDate: '2026-08-01', drawingRef: '', remarks: '' }]);
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/orders', expect.objectContaining({ inquiryId: 'i1' })));
  });
});
...
describe('Sales — complaint + CAPA (API-backed)', () => {
  ...
  const wire = (complaint: unknown) => get.mockImplementation((path: string) => {
    if (path === '/complaints/batches') return Promise.resolve(batches);
    if (path === '/complaints') return Promise.resolve([complaint]);
  ...
  it('resolves once the CAPA is closed', async () => {
    ...
    await waitFor(() => expect(post).toHaveBeenCalledWith('/complaints/x1/resolve'));
  });
});
