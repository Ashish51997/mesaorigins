...
function renderScreen(assignments: unknown[] = []) {
  mocks.get.mockImplementation((path: string) => {
    if (path === '/roles') return Promise.resolve([role]);
    if (path === '/employees') return Promise.resolve([employee]);
    if (path === '/mesaops/role-assignments') return Promise.resolve(assignments);
    if (path === '/screens') return Promise.resolve({ screens: [] });
...
describe('MesaOps plant access administration', () => {
  ...
  it('revokes a versioned assignment with a recorded reason', async () => {
    ...
    await waitFor(() => expect(mocks.postIdempotent).toHaveBeenCalledWith(
      '/mesaops/role-assignments/assignment-1/revoke',
