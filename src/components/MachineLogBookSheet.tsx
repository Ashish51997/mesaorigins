...
function Cell({ w, value, onChange, readOnly, field }: CellProps) {
  ...
  return (
    <input
      ...
      className={isActive ? 'cin active' : 'cin'}
 ...
 * and shows its permissible value as a hint while focused. */
function RangeCell({ lo, hi, value, onChange, w, readOnly, field }: RangeCellProps) {
  ...
  const oor = v.trim() !== '' && !Number.isNaN(n) && (n < lo || n > hi);
  const cls = `${oor ? 'cin oor' : 'cin'}${isActive ? ' active' : ''}`;
  return (
    <span className="rc">
      <input
        ref={isActive && setActiveEl ? setActiveEl : undefined}
      ...
      />
      {focused && <span className="hint">Permissible: {lo} – {hi}</span>}
    </span>
  );
}
  ...
  return (
    <div className="sheet-wrap">
      ...
      <div className="toolbar">
        <button type="button" className="print-btn" onClick={() => window.print()}>Print sheet</button>
      </div>
      ...
      <div className="sheet">
        ...
        <div className="idrow">
          ...
          <span>Date: <Cell w="90px" field="date" value={l.date} onChange={(v) => on.scalar('date', v)} readOnly={readOnly} /></span>
        ...
        <div className="lb-sec">
                ...
                {t.dieZones.map((z) => {
                  ...
                  return (
                    <td key={z}>{ranged
                      ...
                      : <Cell field={`die:${z}`} value={l.dieZoneTemps[z] ?? ''} onChange={(v) => on.dieZone(z, v)} readOnly={readOnly} />}</td>
                  );
                })}
                {t.barrelZones.map((z) => {
                  const zs = t.zoneSpecs?.[z];
                  ...
                  return (
                    <td key={z}>{ranged
                      ...
                      : <Cell field={`barrel:${z}`} value={l.barrelZoneTemps[z] ?? ''} onChange={(v) => on.barrelZone(z, v)} readOnly={readOnly} />}</td>
                  );
                })}
                <td><Cell field="motorSpeed" value={l.motorSpeed} onChange={(v) => on.scalar('motorSpeed', v)} readOnly={readOnly} /></td>
                <td><Cell field="ampere" value={l.ampere} onChange={(v) => on.scalar('ampere', v)} readOnly={readOnly} /></td>
                <td><Cell field="takeupSpeed" value={l.takeupSpeed} onChange={(v) => on.scalar('takeupSpeed', v)} readOnly={readOnly} /></td>
                <td><Cell field="vacuum" value={l.vacuum} onChange={(v) => on.scalar('vacuum', v)} readOnly={readOnly} /></td>
              </tr>
            </tbody>
          </table>
          ...
          <table className="lbgrid tight">
            <tbody>
              <tr>
                <td className="lbl">Extruder start time</td><td><Cell field="extruderStartTime" value={l.extruderStartTime} onChange={(v) => on.scalar('extruderStartTime', v)} readOnly={readOnly} /></td>
                <td className="lbl">Product / Item set time</td><td><Cell field="productSetTime" value={l.productSetTime} onChange={(v) => on.scalar('productSetTime', v)} readOnly={readOnly} /></td>
                <td className="lbl">Shore {t.hardnessType ?? 'A'} Hardness</td><td><Cell field="shoreHardness" value={l.shoreHardness} onChange={(v) => on.scalar('shoreHardness', v)} readOnly={readOnly} /></td>
                <td className="lbl">Production Per Hour</td><td><Cell field="productionPerHour" value={l.productionPerHour} onChange={(v) => on.scalar('productionPerHour', v)} readOnly={readOnly} /></td>
              </tr>
              <tr>
                ...
                {t.inspectionTimeSlots.map((slot, i) => {
                  ...
                  return (
                    <tr key={slot}>
                      ...
                      <td><Cell field={`hourly:${i}:od`} value={insp.od ?? ''} onChange={(v) => on.hourly(i, 'od', v)} readOnly={readOnly} /></td>
                      <td><Cell field={`hourly:${i}:weight`} value={insp.weight ?? ''} onChange={(v) => on.hourly(i, 'weight', v)} readOnly={readOnly} /></td>
                      <td><Cell field={`hourly:${i}:colour`} value={insp.colour ?? ''} onChange={(v) => on.hourly(i, 'colour', v)} readOnly={readOnly} /></td>
                      <td><Cell field={`hourly:${i}:okNotOk`} value={insp.okNotOk ?? ''} onChange={(v) => on.hourly(i, 'okNotOk', v)} readOnly={readOnly} /></td>
                ...
                {t.inspectionTimeSlots.map((slot, i) => {
                  ...
                  return (
                    <tr key={slot}>
                      ...
                      <td><Cell field={`hourly:${i}:perMeter`} value={insp.perMeter} onChange={(v) => on.hourly(i, 'perMeter', v)} readOnly={readOnly} /></td>
                      <td><Cell field={`hourly:${i}:colour`} value={insp.colour} onChange={(v) => on.hourly(i, 'colour', v)} readOnly={readOnly} /></td>
                  ...
                  {range(1, t.traceability.rowsPerTable).map((n) => {
                    ...
                    return isPipe ? (
                      <tr key={n}>
                        ...
                        <td><Cell w="60px" field={`trace:${idx}:pktKg`} value={row.pktKg ?? ''} onChange={(v) => on.trace(idx, 'pktKg', v)} readOnly={readOnly} /></td>
        ...
        <div className="lb-sec">
          ...
          <table className="lbgrid">
            ...
            <tbody>
              <tr>
                <td><Cell field="totalRollsProduced" value={l.totalRollsProduced} onChange={(v) => on.scalar('totalRollsProduced', v)} readOnly={readOnly} /></td>
                <td><Cell field="totalRollKgs" value={l.totalRollKgs} onChange={(v) => on.scalar('totalRollKgs', v)} readOnly={readOnly} /></td>
                <td><Cell field="processWasteKg" value={l.processWasteKg} onChange={(v) => on.scalar('processWasteKg', v)} readOnly={readOnly} /></td>
                <td><Cell field="lumpsWasteKg" value={l.lumpsWasteKg} onChange={(v) => on.scalar('lumpsWasteKg', v)} readOnly={readOnly} /></td>
                <td><Cell field="rejectionKg" value={l.rejectionKg} onChange={(v) => on.scalar('rejectionKg', v)} readOnly={readOnly} /></td>
                <td><Cell field="totalConsumedKg" value={l.totalConsumedKg} onChange={(v) => on.scalar('totalConsumedKg', v)} readOnly={readOnly} /></td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="rej">
            ...
            {t.rejectionReasons.map((r, i) => (
              <span className="rej-item" key={r}>{i + 1}) {r} <Cell w="42px" field={`rej:${r}`} value={l.rejectionCounts[r] ?? ''} onChange={(v) => on.rejection(r, v)} readOnly={readOnly} /></span>
            ))}
          </div>
          ...
          <table className="lbgrid tight">
            <tbody>
              <tr>
                ...
                <td className="lbl">Time</td><td><Cell field="meterCheckTime" value={l.meterCheckTime} onChange={(v) => on.scalar('meterCheckTime', v)} readOnly={readOnly} /></td>
                <td className="lbl">Meter</td><td><Cell field="meter" value={l.meter} onChange={(v) => on.scalar('meter', v)} readOnly={readOnly} /></td>
                <td className="lbl">Meter Count Set</td><td><Cell field="meterCountSet" value={l.meterCountSet} onChange={(v) => on.scalar('meterCountSet', v)} readOnly={readOnly} /></td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="sign">
            <div>(Signature of Inspector)</div>
            <div>(Signature of Shift Incharge)</div>
          </div>
        </div>
      </div>
      </FieldCtx.Provider>
...
.sheet{max-width:920px;margin:0 auto;background:#fff;border:2px solid #111;padding:10px 12px 14px;font-size:12px}
.sheet *{box-sizing:border-box}

.cin{border:0;background:transparent;font-family:inherit;font-size:12px;width:100%;padding:1px 2px;outline:none;color:#0a2a6b}
...
.drop-btn{display:inline-flex;align-items:center;justify-content:space-between;gap:4px;width:100%;min-height:22px;border:0;background:transparent;font-family:inherit;font-size:12px;cursor:pointer;padding:1px 2px;color:#0a2a6b;border-radius:3px}
...
.trace-grid td{height:22px;text-align:left}
