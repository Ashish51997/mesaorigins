...
export function useCustomers() {
  return useQuery({ queryKey: keys.customers, queryFn: () => api.get<ApiCustomer[]>('/customers') });
}
export function useCreateCustomer() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Partial<ApiCustomer>) => api.post<ApiCustomer>('/customers', body),
...
export function useInquiries() {
  return useQuery({ queryKey: keys.inquiries, queryFn: () => api.get<ApiInquiry[]>('/inquiries') });
}
export function useCreateInquiry() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Record<string, unknown>) => api.post<ApiInquiry>('/inquiries', body),
...
export function useQuoteInquiry() {
  ...
  return useMutation({
    mutationFn: ({ id, quotationPrice }: { id: string; quotationPrice: number }) =>
      api.post<ApiInquiry>(`/inquiries/${id}/quote`, { quotationPrice }),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.inquiries }),
  });
}
...
export function useOrders() {
  return useQuery({ queryKey: keys.orders, queryFn: () => api.get<ApiOrder[]>('/orders') });
}
export function useConfirmOrder() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: { inquiryId: string; priority?: string }) => api.post<ApiOrder>('/orders', body),
    onSuccess: () => {
...
export function useCancelOrder() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.post<{ ok: boolean }>(`/orders/${id}/cancel`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: keys.orders });
      qc.invalidateQueries({ queryKey: keys.inquiries });
    },
...
export function useMembers() {
  return useQuery({ queryKey: keys.members, queryFn: () => api.get<ApiMember[]>('/members') });
}
