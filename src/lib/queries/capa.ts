...
export function useComplaintBatches() {
  return useQuery({ queryKey: keys.batches, queryFn: () => api.get<ApiBatch[]>('/complaints/batches') });
}
export function useComplaints() {
  return useQuery({ queryKey: keys.complaints, queryFn: () => api.get<ApiComplaint[]>('/complaints') });
}
...
export function useLogComplaint() {
  ...
  return useMutation({
    mutationFn: (body: { salesOrderId: string; severity: string; description: string; photoUrl?: string }) =>
      api.post<ApiComplaint>('/complaints', body),
...
export function useResolveComplaint() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.post<ApiComplaint>(`/complaints/${id}/resolve`),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.complaints }),
  });
}
...
export function useUpdateCapa() {
  ...
  return useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<Pick<ApiCapa, 'rootCause' | 'correctiveAction' | 'preventiveAction' | 'responsiblePerson' | 'dueDate'>> }) =>
      api.patch<ApiCapa>(`/capas/${id}`, patch),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.complaints }),
  });
}
export function useCloseCapa() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.post<ApiCapa>(`/capas/${id}/close`),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.complaints }),
  });
}
