...
export function useSchedulePlan() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Record<string, unknown>) => api.postIdempotent<ApiPlan>('/plans', body, `production-plan:${crypto.randomUUID()}`),
    onSuccess: () => invalidatePlanning(qc),
  });
}
export function useUpdatePlan() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) => api.patchIdempotent<ApiPlan>(`/plans/${id}`, body, `production-plan-update:${crypto.randomUUID()}`),
    onSuccess: () => invalidatePlanning(qc),
  });
}
